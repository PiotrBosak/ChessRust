use crate::{Advance, Board, Color, Position};
...
pub fn possible_move_positions(
    ...
    let mut vec = vec![];
    while let Some(position) = next_position.filter(|pos| board.tile_at(pos).is_empty()) {
        vec.push(position);
...
pub fn possible_capture_positions<'a>(
    ...
    while let Some(pos) = posistion {
        match board.tile_at(&pos).current_piece {
            ...
            None => posistion = find_next_position(pos, next_rank_diff, next_file_diff),
        }
    }
    None
}
