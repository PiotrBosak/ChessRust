...
use crate::logic::{Advance, Color, Move, Piece, PieceType, Position, Rank, Tile};
use crate::MoveType;
...
impl Board {
    ...
    pub fn make_move(&self, from: &Position, to: &Position) -> Option<Board> {
        ...
        let move_ = valid_moves.into_iter().find(|m| m.to == *to)?;
        let new_board = self.move_(&move_).map(|board| Board {
            tiles: board.tiles,
            previous_move: Some(move_),
        })?;
        Some(new_board)
    }
    ...
    fn move_(&self, move_: &Move) -> Option<Board> {
        match &move_.move_type {
            ...
            MoveType::Castling => self.make_castling(&move_.from, &move_.to),
            MoveType::LePassant => self.make_le_passant(&move_.from, &move_.to),
        }
    }
    ...
    fn make_castling(&self, from: &Position, to: &Position) -> Option<Board> {
        ...
        let rook_file = if from.file == File::B {
            File::A
        } else {
            File::H
        };
        let new_rook_file = if from.file == File::B {
            File::C
        } else {
            File::F
        };
        ...
        new_board
            .tiles
            .insert(*from, Tile::empty(*from).mark_moved());
        new_board
            .tiles
            .insert(*to, Tile::with_piece(*to, king_piece).mark_moved());
        new_board
            .tiles
            .insert(rook_position, Tile::empty(rook_position).mark_moved());
        new_board.tiles.insert(
            new_rook_position,
            Tile::with_piece(new_rook_position, rook_piece).mark_moved(),
        );
    ...
    fn make_le_passant(&self, from: &Position, to: &Position) -> Option<Board> {
        ...
        let difference = if attacker_piece.color == Color::White {
            -1
        } else {
            1
        };
        ...
        new_board
            .tiles
            .insert(captured_position, captured_tile.mark_moved());
    ...
    fn create_tiles_with_black_bishops() -> HashMap<Position, Tile> {
        let mut tiles = HashMap::new();
        tiles.insert(
            Position::new(File::C, Rank::Eight),
            Tile::with_piece(
                Position::new(File::C, Rank::Eight),
        ...
        tiles.insert(
            Position::new(File::F, Rank::Eight),
            Tile::with_piece(
                Position::new(File::F, Rank::Eight),
                Piece::new(PieceType::Bishop, Color::Black),
            ),
    ...
    fn create_tiles_with_black_knights() -> HashMap<Position, Tile> {
        let mut tiles = HashMap::new();
        tiles.insert(
            Position::new(File::B, Rank::Eight),
            Tile::with_piece(
                Position::new(File::B, Rank::Eight),
    ...
    fn create_tile_with_black_king() -> HashMap<Position, Tile> {
        let mut tiles = HashMap::new();
        tiles.insert(
            Position::new(File::E, Rank::Eight),
            Tile::with_piece(
                Position::new(File::E, Rank::Eight),
